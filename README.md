...
The Kandy HID Toolkit currently supports the following Jabra headsets, with support for HID devices from other vendors possible.<br>
...
Add the tgz file to your workspace and to your package.json. See more details in [CHANGELOG](./CHANGELOG.md).
...
Logs generated from the HID DLL running on the Thin Client are written to syslog, and so can be observed in /var/log/messages. They also have a prefix of `VDIHID:`
...
```
    ...
    RTC_HID=Off
```
Note that this will prevent kandy-hid within the container/web app from communicating with HID devices.
